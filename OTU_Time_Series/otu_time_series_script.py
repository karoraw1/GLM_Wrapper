...
print "\nTransforming counts into centered log ratios"
...
less_div_path = "less_div_reps.p"
do_rep_rep = False
if not os.path.exists(less_div_path) or do_rep_rep:
    print "\nMeasuring B-diversity (sqrt(JSD)) distances between replicates"
    less_diverse_reps, broken_groups = ReplicateReport(only_depths_n, 
                                                       only_depth_otus, 
                                                       rep_groups, False, "JSD")
        
    kept_pct = broken_groups/float(len(rep_groups))
    print "\nGroups with replicates removed: {0:.3f}%".format(kept_pct*100)
    pickle.dump( less_diverse_reps, open( less_div_path, "wb" ) )
else:
    print "\tLoading pre-approved replicates"
    less_diverse_reps = pickle.load( open( less_div_path , "rb" ) )
    

print "\nDropping less diverse replicates"

derepped_clr_m = clr_T_m.drop(less_diverse_reps)
dereplicated_clr = clr_T_otus.drop(less_diverse_reps)

derepped_otu_m = only_depths_n.drop(less_diverse_reps)
dereplicated_otu = only_depth_otus.drop(less_diverse_reps)
new_rep_groups = dropBadReps(less_diverse_reps, rep_groups)

print "\nPerforming PCA on dereplicated OTU matrix of log ratios"

_ = scalePCAcorrelate(dereplicated_clr, derepped_clr_m, metadata, True)
...
for i in ingroup_otu_m.index:
    ...
    alpha_plot_df.ix[this_depth, this_date] = ingroup_otu_m.enspie[i]

alpha_plot_df.fillna(value=0, inplace=True)
...
shared_vst = varStabTransform(to_transform, shared_otus)

print "\n Performing PCA on high frequency log ratios" 

PCA_object = scalePCAcorrelate(shared_clr, shared_clr_m, metadata, True)
X_std2 = shared_clr.values
...
with plt.style.context('seaborn-whitegrid'):
    ...
    for lab, lab_en, col in zip(('Q1', 'Q2', 'Q3', 'Q4'),range(1,5),
        ...
        plt.scatter(y_std2[(shared_otus_m.Quadrants==lab_en).values, 0],
                    y_std2[(shared_otus_m.Quadrants==lab_en).values, 1],
                    label=lab,
                    c=col)
...
if loaded_otu.equals(shared_otus):
    ...
    if os.path.exists(bc_dist_path):
        ...
        bc_dist_mat = pd.read_csv(bc_dist_path, index_col=0)
    else:
        print "Calculating sqrt of Jensen Shannon Divergence of all rows"
