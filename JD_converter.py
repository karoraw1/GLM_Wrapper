# -*- coding: utf-8 -*-
"""
Created on Sat Apr 16 13:32:01 2016
...
def base60_to_decimal(xyz,delimiter=None):
 ...
 """
  ...
  for i,j in zip(xyzlist,divisors): # if xyzlist has❤ values then
...
def caldate(mjd):
 ...
 """
  ...
  date1 = str(int(year))+"-"+str(int(month))+"-"+str(int(day))+" "
  date2 = date1+str(int(sign+str(hour)))+":"+str(minute)+":"
  date3 = date2+"{0:.2f}".format(second)
  return date3
